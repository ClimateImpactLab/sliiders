...
CRED_PATH = AnyPath("/opt/gcsfuse_tokens/rhg-data.json")
...
client = GSClient(application_credentials=CRED_PATH)
bucket = client.GSPath(f"gs://{CRED_PATH.stem}")
DIR_SCRATCH = client.GSPath(f"gs://{CRED_PATH.stem}-scratch/sliiders")
FS = filesystem("gcs", project=CRED_PATH.stem, token=str(CRED_PATH))
DIR_DATA = bucket / "impactlab-rhg/coastal/sliiders"
DIR_SHAPEFILES = bucket / "impactlab-rhg/spatial/shapefiles/source"
...
DIR_GADM = DIR_SHAPEFILES / "gadm" / GADM_VERS_STR
...
DIR_ELEVATION = bucket / "impactlab-rhg/common_data/elevation"
